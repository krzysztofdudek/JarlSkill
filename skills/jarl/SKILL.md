...
| `.jarl/decisions.md` | Rulings, append-only: `## YYYY-MM-DD · slug`, the ruling, **By:** who ruled. Read before deciding anything; never re-derive a ruling that is here. A ruling marked **Superseded by:** is no longer in force — the one that superseded it is; `decisions --live` lists only the rulings in force. | jarl |
...
| `review <ids> approve\|changes "<findings>" [--by <reviewer>]` | the reviewer's verdict; `changes` needs a finding ranked Critical or Important in the text — Minor alone cannot bounce a branch, it rides an approve into evidence; an approve needs `--by`, the reviewer's name, and the log line records it with its kind — `fresh`, `coordinator` (`--by jarl`) or `self` (the issue's worker, from **Worker:** or its last lease); an approve by the worker is refused, because a self-approve is not a review; `done` refuses without an approve newer than the last round and the last reopen (a move from done or in progress back to open, or any move out of done); `status` and `report` print who reviewed the done work (`fresh a · coordinator b · self c · unrecorded d`, unrecorded for approves from before `--by`); the output repeats the issue's acceptance, so the verdict is read against it |
...
4. Green: `jarl.mjs merged <ids> --sha <merge sha>` (`--ci none` when the repository has no CI; else
   `jarl.mjs merged <ids> --ci green|red` once it reports), `jarl.mjs evidence <id> "<check summary line, merge sha>"`, `jarl.mjs set <id> done`,
